...
#==============================================================================
# RPi GPIO Power OFF Script (Host-Side)
# Triggers GPIO on Raspberry Pi via SSH to turn power OFF
#
# Usage:
#   ./gpio_off.sh -c plp.conf
#   ./gpio_off.sh plp.conf                 # legacy shorthand
#   ./gpio_off.sh                          # Uses defaults
#==============================================================================
...
usage() {
        cat <<'EOF'
Usage: gpio_off.sh [options]

Options:
    -c, --config <file>   Load configuration (same format as plp.conf)
    -h, --help            Show this help message

Legacy positional use is still supported: gpio_off.sh plp.conf
EOF
}
...
if [ "$GPIO_MODE" = "aardvark" ]; then
    ...
    if [ -n "$CONFIG_FILE" ]; then
        exec bash "${SCRIPT_DIR}/scripts/aardvark/gpio_off_aardvark.sh" -c "$CONFIG_FILE"
    else
        exec bash "${SCRIPT_DIR}/scripts/aardvark/gpio_off_aardvark.sh"
    fi
    exit $?
...
print_info "========================================"
print_info "RPi GPIO Power OFF"
print_info "========================================"
print_info "Target: ${RPI_USER}@${RPI_HOST}"
print_info "GPIO Pin: ${RPI_GPIO_PIN} (BCM)"
print_info "Sequence: PCIe remove -> export -> set HIGH (power OFF)"
echo ""
...
if [ -d "$OP_GPIO_PATH" ]; then
    log "GPIO${GPIO_PIN} already exported -> unexporting"
...
log "Drive HIGH (power OFF)"
echo 1 > $OP_GPIO_PATH/value

log "GPIO${GPIO_PIN} remains exported to hold HIGH state"
EOF
)
...
if run_remote_sequence; then
    print_info "✓ GPIO${RPI_GPIO_PIN} power-off sequence completed"
    ...
    print_error "✗ Failed to execute GPIO${RPI_GPIO_PIN} power-off sequence"
    exit 1
fi
